...
GROUP BY articles.title ORDER BY popularity DESC LIMIT 3")
tops = c.fetchall()
db.close()
print "The three most popular atricles of all time are:"
for article in tops:
...
ranks = c.fetchall()
db.close()
print "Authors ranked by article views below:"
for author in ranks:
...
db = psycopg2.connect("dbname=news")
c = db.cursor()
c.execute("SELECT errors.date, (1.0 * errors.failures/requests.attempts)\
* 100 as percent_fails FROM errors INNER JOIN requests\
ON (errors.date = requests.date) WHERE (1.0*errors.failures/\
requests.attempts) * 100 > 1.0")
above1 = c.fetchall()
db.close()
print "Date(s) with error rate above 1%:"
for day in above1:
